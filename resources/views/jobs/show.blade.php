...
#applicant-form{
    ...
    <div class= "grid grid-cols-1 md:grid-cols-4 gap-6">
        <section class="md:col-span-3">
            ...
            <div class="flex justify-between bg-white p-3">    
                <a class="block p-4 text-blue-700" href="{{ route('jobs.index') }}">
                    <i></i>Back to Listings        
                </a>
                @can('update',$job)
                <div class="flex space-x-3 ml-4">
                    <a  class= "px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded" href="{{ route('jobs.edit',$job->id) }}">
                    <i></i>Edit
                    </a>
                    ...
                    <button
                        ...
                        text-white rounded">Delete</button>
                    </form>
                </div>
                @endcan
            </div>
            ...
            <div class="container mx-auto p-4">
                ...
                <div class= "rounded-lg shadow-md bg-white p-4">
                    ...
                    <h3 class="text-lg font-semibold mb-2 text-blue-500" >Job Benefits </h3>
                    <p>{{$job->benefits}}</p>
                </div>  
                @endif
            ...
            <div x-data="{ open: false }" id="applicant-form">
                ...
                shadow-sm rounded border text-base font-medium
                cursor-pointer text-indigo-700 bg-indigo-100
                hover:bg-indigo-200" >Apply Now</button>

                <div x-cloak x-show="open"
                class="fixed inset-0 flex items-center justify-center
                bg-gray-900 bg-opacity-50 z-[9999]">
                <div @click.away="open = false" class="bg-white p-6 rounded-lg shadow-md w-full max-w-md">
                    ...
                    enctype="multipart/form-data">
                        @csrf
                        <x-input-link id="full_name" name="full_name" label="Full Name" value="{{ auth()->user()->name }}" :required="true"/>
                        <x-input-link id="contact_phone" name="contact_phone" label="Contact phone" value="" />
                        <x-input-link id="contact_email" name="contact_email" label="Email" value="{{ auth()->user()->email }}" :required="true"/>
                        <x-text-link id="message" name="message" label="Message"/>
                        <x-input-link id="location" name="location" label="Location" value=""/>
                       
                        <x-image-link id="resume" name="resume" label="Upload Resume (pdf)"  :required="true"/>
                        <button type="submit" 
                        class="block w-full text-center px-5 py-3
                        shadow-sm rounded border text-base font-medium
                        cursor-pointer text-indigo-700 bg-indigo-100
                        hover:bg-indigo-200">Submit</button>

                        <button @click="open = false" type="submit"
                        class="block w-full text-center px-5 py-3
                        shadow-sm rounded border text-base font-medium
                        cursor-pointer text-white-700 bg-red-500
                        hover:bg-red-600">Cancel</button>
                    </form>
                </div>

            </div>
            @else
                <p class="my-5" >You must be logged in to apply fo a job!</p>
            @endauth

            <div class="bg-white rounded-ls shadow-md p3">
                <div id="map"></div>
            </div>
        ...
        <aside class="bg-white rounded-lg shadow-md p-3">
                ...
                alt="{{$job->company_name}}" class="w-full 
                rounded-lg mb-4 m-auto" />
                @endif
                ...
                @guest()
                    <p class="mt-10 bg-gray-200 text-gray-700 font-bold w-full py-2 px-4 rounded-full text-center">Login necessary!</p>
                @else

                    <form method="POST" action="{{auth()->user()->bookmarkedJobs()->where('job_id',$job->id)->exists()
                    ?route('bookmarks.destroy',$job->id)
                    :route('bookmarks.store',$job->id)}}" class="mt-10">
